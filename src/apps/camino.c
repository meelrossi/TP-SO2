...
typedef struct
{
	unsigned char road[2][ROAD_LEN];
	Semaphore_t *mail_empty, *mail_full;
	MESSAGE mail;
	unsigned ncars;
}
data;

#define road		TLS(data)->road
#define mail_empty 	TLS(data)->mail_empty
#define mail_full	TLS(data)->mail_full
#define mail		TLS(data)->mail
#define ncars		TLS(data)->ncars
...
send_car(DIRECTION dir)
{
	...
	Ready(CreateTask(car, 0, dir == LEFTBOUND ? &left : &right, "car", DEFAULT_PRIO));
}

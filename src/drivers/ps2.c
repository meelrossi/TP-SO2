...
static bool
mouse_send(unsigned char cmd)
{
	kbd_send_ctl(MSECMD);
	kbd_send(cmd);
	return kbd_receive() == MSEACK;
}
...
init_mouse(void)
{
	// Leer teclas que pueda haber acumuladas (Qemu)
	while ( kbd_receive() != -1 )
		;
	...
	// Detectar si es un Intellimouse
	intelli = 	mouse_send(MSESETRATE) && mouse_send(200) &&
				mouse_send(MSESETRATE) && mouse_send(100) &&
				mouse_send(MSESETRATE) && mouse_send(80)  &&
				mouse_send(MSEGETID)   && kbd_receive() == 0x03;

	if ( intelli )
		print0("ps2: Intellimouse detectado\n");
	else
		mouse_send(MSEDEFAULT);

	// Habilitar envío de datos en modo stream
	mouse_send(MSESTREAM);
}

...
print_circle(int acum_x, int acum_y, int selected_color, mouse_event_t m_event) {
	...
	if (m_event.left_button) {
		...
		print_erasable_circle(fixed_x, fixed_y, prev_acum_x, prev_acum_y, prev_radius, prev_center_x, prev_center_y, selected_color, ERRASE);
		print_erasable_circle(fixed_x, fixed_y, acum_x, acum_y, radius, center_x, center_y, selected_color, TRANSITORY);
		...
		print_erasable_circle(fixed_x, fixed_y, acum_x, acum_y, radius, center_x, center_y, selected_color, DEFINITIVE);
		fixed_x = -1;
...
void
print_erasable_circle(int fixed_x, int fixed_y, int acum_x, int acum_y, int radius, int center_x, int center_y, int selected_color, int type){
	int i,j;
	for(i = fixed_x; i < acum_x; i++){
		for(j = fixed_y; j < acum_y; j++){
			if (distance(i, j, center_x, center_y) == radius){
				if(type == ERRASE)
					print_prev_px(i, j);

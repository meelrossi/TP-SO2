...
mt_main(unsigned magic, boot_info_t *info)
{
	...
	// Habilitar interrupciones y calibrar lazo de microdelays
	i = msecs_to_ticks(1000);
	print0("Habilitar interrupciones\nCalibrando microdelays\n"); 
	mt_sti();
	while ( timer_ticks < i )
	{
		unsigned n = 1000000;
...
Toma memoria para crear el stack y lo inicializa.
